...
export function ForgotPasswordForm() {
  ...
  if (state.status === "sent") {
    return (
      <div className="flex flex-col gap-2 rounded-2xl border-2 border-green bg-green-soft p-5 text-center">
        ...
        <p className="font-display text-[15px] font-bold text-green-dark">Mail verschickt!</p>
        <p className="text-sm text-ink-soft">
          Falls für {state.message} ein Konto existiert, kommt gleich ein Link zum Zurücksetzen.
        </p>
      </div>
    );
  }
  ...
  return (
    <form action={formAction} className="flex flex-col gap-3">
      ...
      >
        {pending ? "Sendet …" : "Link zum Zurücksetzen senden"}
      </button>
    </form>
  );
}
